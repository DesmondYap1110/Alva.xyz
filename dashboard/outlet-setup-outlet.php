...
<head>
    <meta charset="utf-8">
    <title>Outlet Setup | Outlet Module | <?php echo $base_name; ?></title>
        ...
        <div class="main-content">
            ...
            <section id="bc-section">
                <div id="bc-div">
                    ...
                    Outlet Setup
                    <span id="bc-arrow"><i class="ri-arrow-right-s-line"></i></span>                    
                    <span id="bc-active">Outlet Module</span>
                </div>
            </section>
                ...
                <section id="general-section">
                    <div class="container-fluid">
                        <div class="row">
                            <div class="col-lg-12">
                                <div id="add-btn-div">
                                    <a href="<?php echo $base_url_dashboard; ?>/outlet-setup-outlet-add.php" id="general-btn" class="btn1"><i class="ri-add-fill"></i>Add Outlet</a>
                                </div>
                                <div id="tb-box" class="general-box">
                                    <div id="tb-border-line">
                                        <div id="tb-title-div">
                                            <p id="tb-title">Outlet Module</p>
                                            <?php if (isset($_GET['add'])) { $register = $_GET['add']; if ($register == 'succ') { ?>
                                            <div class="alert alert-success" role="alert">
                                                <p class="alert-heading">Outlet Added Successfully.</p>
                                            </div>
                                            <?php } } ?>
                                            <?php if (isset($_GET['delete'])) { $register = $_GET['delete']; if ($register == 'succ') { ?>
                                            <div class="alert alert-success" role="alert">
                                                <p class="alert-heading">Outlet Deleted Successfully.</p>
                                            </div>
                                            <?php } } ?>
                                            <?php if (isset($_GET['update'])) { $register = $_GET['update']; if ($register == 'succ') { ?>
                                            <div class="alert alert-success" role="alert">
                                                <p class="alert-heading">Outlet Edited Successfully.</p>
                                            </div>
                                            <?php } } ?>
...
function displaydata($data,$delete)
{
    ...
    if($total_row>0)
    {
        ...
        for($i=0; $i<$total_row; $i++)
        {
            ...
            '
            <tr>
                <td>'.$data->get_row()[$i]['outlet_ID'].'</td>
                <td>'.$data->get_row()[$i]['name'].'</td>
                <td>'.$data->get_row()[$i]['location'].'</td>
                <td>
                '.
                managestatus( 
                    $data->get_row()[$i]['ID'] , 
                ...
                <td>
                    <div id="tb-act-btn-div">
                        <a href="'.$GLOBALS['base_url_dashboard'].'/outlet-setup-outlet-edit.php?oid='.$data->get_row()[$i]['outlet_ID'].'" class="tb-ac-btn" id="tb-ac-btn-1" data-bs-toggle="tooltip" data-bs-placement="top" data-bs-original-title="Edit Outlet" ><i class="ri-edit-2-line"></i></a>
...
function managestatus($ID,$status,$togo)
{
    if($status==1)
    {
        return 
        '
        <a href="status.php?oid='.$ID.'&status=0&togo='.$togo.'" id="general-btn" class="btn3" data-bs-toggle="tooltip" data-bs-placement="top" data-bs-original-title="Click to Block">Active</a>
        ';
    }
    elseif($status==0)
    {
        return
        '
        <a href="status.php?oid='.$ID.'&status=1&togo='.$togo.'" id="general-btn" class="btn5" data-bs-toggle="tooltip" data-bs-placement="top" data-bs-original-title="Click to Active">BLOCK</a>
        ';
    }

 <?php
    ...
    $delete = new Modal(
        ...
        //Title Modal
        "Delete Operation Manager",

        //Modal Description
        "delete this Operation Manager?",
...
<head>
    <meta charset="utf-8">
    <title>Outlet Setup | Operation Manager Module | <?php echo $base_name; ?></title>
        ...
        <div class="main-content">
            ...
            <section id="bc-section">
                <div id="bc-div">
                    ...
                    Outlet Setup
                    <span id="bc-arrow"><i class="ri-arrow-right-s-line"></i></span>                    
                    <span id="bc-active">Operation Manager Module</span>
                </div>
            </section>
                ...
                <section id="general-section">
                    <div class="container-fluid">
                        <div class="row">
                            <div class="col-lg-12">
                                <div id="add-btn-div">
                                    <a href="<?php echo $base_url_dashboard; ?>/outlet-setup-operation-add.php" id="general-btn" class="btn1"><i class="ri-add-fill"></i>Add Operation Manager</a>
                                </div>
                                <div id="tb-box" class="general-box">
                                    <div id="tb-border-line">
                                        <div id="tb-title-div">
                                            <p id="tb-title">Operation Manager Module</p>
                                            <?php if (isset($_GET['add'])) { $register = $_GET['add']; if ($register == 'succ') { ?>
                                            <div class="alert alert-success" role="alert">
                                                <p class="alert-heading">Operation Manager Added Successfully.</p>
                                            </div>
                                            <?php } } ?>
                                            <?php if (isset($_GET['delete'])) { $register = $_GET['delete']; if ($register == 'succ') { ?>
                                            <div class="alert alert-success" role="alert">
                                                <p class="alert-heading">Operation Manager Deleted Successfully.</p>
                                            </div>
                                            <?php } } ?>
                                            <?php if (isset($_GET['update'])) { $register = $_GET['update']; if ($register == 'succ') { ?>
                                            <div class="alert alert-success" role="alert">
                                                <p class="alert-heading">Operation Manager Edited Successfully.</p>
                                            </div>
                                            <?php } } ?>
...
function displaydata($data,$delete,$dbconn)
{
    $total_row = $data->totaldata(" AND type='5'");
    ...
    if($total_row>0)
    {
        ...
        for($i=0; $i<$total_row; $i++)
        {
            ...
            <tr>
                ...
                <td>'.getLocation($data->get_row()[$i]['outlet_ID'],$dbconn).'</td>
                <td>'.getservice($data->get_row()[$i]['member_ID'],$dbconn).'</td>
                <td>
                '.
                managestatus( 
                    $data->get_row()[$i]['ID'] , 
                    $data->get_row()[$i]['status'],
                    "outlet-setup-operation"
                )
                .'
                </td>
                <td>
                    <div id="tb-act-btn-div">
                        <a href="'.$GLOBALS['base_url_dashboard'].'/outlet-setup-operation-edit.php?mid='.$data->get_row()[$i]['member_ID'].'" class="tb-ac-btn" id="tb-ac-btn-1" data-bs-toggle="tooltip" data-bs-placement="top" data-bs-original-title="Edit Operation Manager" ><i class="ri-edit-2-line"></i></a>
...
function getLocation($location,$dbconn)
{
    $sql="SELECT * FROM `outlet_module` WHERE outlet_ID='$location'";
...
function getservice($ID,$dbconn)
{
    $sql="SELECT * FROM `member_service` WHERE member_ID='$ID' AND status='1'";
    $query=mysqli_query($dbconn,$sql) or die(mysqli_error($dbconn));
...
function servicedecode($data)
{
    if($data=='1'){return "Tint";}
    elseif($data=='2'){return "Coat";}
    elseif($data=='3'){return "Aircond";}
    elseif($data=='4'){return "Paint";}
    elseif($data=='5'){return "Wrap";}
}
